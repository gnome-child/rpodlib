#![allow(unused)]

use sha1::{Digest, Sha1};

pub(crate) fn generate_hash58(fwid_hex: &str, itunesdb: &[u8]) -> Result<[u8; 20], &'static str> {
    if fwid_hex.len() != 16 {
        return Err("FWID must be 16 chars");
    }

    let mut fwid = [0u8; 8];

    for (j, chunk) in fwid_hex.as_bytes().chunks(2).enumerate() {
        fwid[j] = u8::from_str_radix(std::str::from_utf8(chunk).unwrap(), 16)
            .map_err(|_| "FWID contains non-hex digits")?;
    }

    Ok(generate_hash(&fwid, itunesdb))
}

fn generate_hash(fwid: &[u8; 8], itunesdb: &[u8]) -> [u8; 20] {
    let mut key = [0u8; 64];
    generate_key(fwid, &mut key);

    // --- First SHA1 -----------------------------------------------------------------
    for b in &mut key {
        *b ^= 0x36;
    }
    let mut sha = Sha1::new();
    sha.update(&key);
    sha.update(itunesdb);
    let hash1 = sha.finalize_reset(); // 20 bytes

    // --- Second SHA1 ----------------------------------------------------------------
    for b in &mut key {
        *b ^= 0x36 ^ 0x5C; // flip 0x36 → 0x5c
    }
    sha.update(&key);
    sha.update(&hash1);
    let result = sha.finalize();

    let mut out = [0u8; 20];
    out.copy_from_slice(&result[..]);
    out
}

fn generate_key(fwid: &[u8; 8], key_out: &mut [u8; 64]) {
    // ---- Build the 16-byte y buffer -----------------------------------------------
    let mut y = [0u8; 16];

    for i in 0..4 {
        let a = fwid[i * 2] as u16;
        let b = fwid[i * 2 + 1] as u16;
        let lcm = lcm(a, b);

        let hi = ((lcm >> 8) & 0xFF) as u8;
        let lo = (lcm & 0xFF) as u8;

        let base = i * 4;
        y[base + 0] = TABLE1[hi as usize].wrapping_mul(0xB5).wrapping_sub(3);
        y[base + 1] = TABLE2[hi as usize].wrapping_mul(0xB7).wrapping_add(0x49);
        y[base + 2] = TABLE1[lo as usize].wrapping_mul(0xB5).wrapping_sub(3);
        y[base + 3] = TABLE2[lo as usize].wrapping_mul(0xB7).wrapping_add(0x49);
    }

    // ---- Byte-wise inverse table ---------------------------------------------------
    for byte in &mut y {
        *byte = INV_TABLE[*byte as usize];
    }

    // ---- SHA1(fixed ‖ y) ----------------------------------------------------------
    let mut sha = Sha1::new();
    sha.update(&FIXED);
    sha.update(&y);
    let digest = sha.finalize();

    // Copy the 20-byte digest to the beginning of the 64-byte key buffer.
    key_out[..20].copy_from_slice(&digest[..]);
}

fn gcd(mut a: u16, mut b: u16) -> u16 {
    while b != 0 {
        let tmp = b;
        b = a % b;
        a = tmp;
    }
    a
}

fn lcm(a: u16, b: u16) -> u16 {
    if a == 0 || b == 0 {
        1
    } else {
        (a / gcd(a, b)).saturating_mul(b)
    }
}

pub(crate) const INV_TABLE: [u8; 256] = [
    ,
];

pub(crate) const TABLE1: [u8; 256] = [
    ,
];

pub(crate) const TABLE2: [u8; 256] = [
    ,
];

pub(crate) const FIXED: [u8; 18] = [
    0x67, 0x23, 0xFE, 0x30, 0x45, 0x33, 0xF8, 0x90, 0x99, 0x21, 0x07, 0xC1, 0xD0, 0x12, 0xB2, 0xA1,
    0x07, 0x81,
];
